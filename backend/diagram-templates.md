...
In CodeVision each API endpoint (REST/SOAP/legacy) receives its own sequence diagram plus a matching entry in `callFlows`. The diagram metadata includes the HTTP method + path (or SOAP operation) and whether the flow includes external dependency calls so the UI can offer per-endpoint filtering between internal-only and full flows. Every arrow label comes from the recorded method invocation (`Class.method()`), self-calls are rendered as loops on the same participant, and repository/DAO steps fan out to the shared `Database` participant with the actual CRUD methods listed in the message text.
