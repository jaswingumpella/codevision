...
Sequence Diagram(s): For key call flows, the PlantUML for sequence diagrams is included. There might be one main sequence (e.g., the primary use case of the system) or multiple sequences for different endpoints. Each sequence diagram text is labeled or captioned accordingly, and the messages are written using the captured method names (e.g., `OrderService.validate()`), including self-loop notation and the DAO-to-Database callouts, mirroring what appears in the UI.
